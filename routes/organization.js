...
function getOrganizationData(org) {
  return {
    id: org.get('id'),
  ...
  app.post('/api/new_organization', function(req, res) {
    ...
    models.Organization.forge({
      ...
      .then(function(org) {
        return res.json({
          error: null,
          organization: getOrganizationData(org)
        });
      })
      .catch(function(err) {
        if (err.code == apiHelpers.UNIQUE_VIOLATION) {
          return res.json({
            error: 'This organization already exists. Please login with your ' +
              req.body.domain + ' email address.'
          });
        }
